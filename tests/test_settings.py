...
def test_settings_uses_dispatch_data_dir_from_environment(monkeypatch) -> None:
    data_dir = Path("/tmp/dispatch-test-data")
    monkeypatch.setenv("DISPATCH_DATA_DIR", str(data_dir))

    settings = Settings()

    assert settings.data_dir == data_dir
    assert settings.projects_dir == data_dir / "projects"
    assert settings.config_dir == data_dir / "config"
...
def test_get_secret_returns_env_value_and_none_for_missing(monkeypatch) -> None:
